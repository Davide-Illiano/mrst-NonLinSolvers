        ...
        function [state, failure, report] = solveMinistep(solver, model, state, state0, dt, drivingForces)
            ...
            for i = 1:(maxIts + 1)
                ...
                if m_i == 1
                    ...
                    b = - res(:,size(res,2)); 
                    [Q, R] = qr(A,0); % take reduced QR one
                    a = R^-1 * Q' * b;
                    a(m_i) = 1 - sum(a);
                ...
                % previous solutions
                if size(stepReport.Residuals,2) == 1   % only pressure
                ...
                state.pressure = updated_sum(1:size(updated_sum,1)/2);
                state.c = updated_sum(size(updated_sum,1)/2+1:size(updated_sum,1));
                end
