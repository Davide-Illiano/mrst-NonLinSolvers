classdef NonLinearSolverAnderson < handle
    ...
    properties
        ...
        analyticalForce1
        analyticalForce2
        m  % number of previus iterations taken into account
    end
    ...
    methods
        function solver = NonLinearSolverAnderson(varargin)
            solver.maxIterations   = 25;  %25
        ...
        function [state, failure, report] = solveMinistep(solver, model, state, state0, dt, drivingForces)
            ...
            for i = 1:(maxIts + 1)
                ...
                % Choose m
                if model.Anderson == 1
                M = 3;    % we use only three previously computed solutions
                m_i = min(i,M);
                % Save the report of each iteration
                
                if  size(stepReport.Residuals,2) == 1
                   res(:,m_i) = cell2mat(stepReport.LinearSolver.VectorResiduals(1));
                   prev(:,m_i) = stepReport.LinearSolver.state_prev.pressure;
                else
                   res(:,m_i) = [cell2mat(stepReport.LinearSolver.VectorResiduals(1))' cell2mat(stepReport.LinearSolver.VectorResiduals(2))'];
                   prev(:,m_i) = [stepReport.LinearSolver.state_prev.pressure' stepReport.LinearSolver.state_prev.c'];
                end
                ...
                if m_i == 1
                    a = 1;
                     ...
                     for j = 1:size(res,2)-1
    					  A(:,j) = (res(:,j) - res(:,size(res,2)));
					 end   % work on it [r_1 - r_n, r_2 - r_n, ... , r_n-1 - r_n]
                    b = - res(:,size(res,2)); % ok
                    [Q, R] = qr(A,0); % take reduce one, only same colums as A
                    a = R^-1 * Q' * b;
                    a(m_i) = 1 - sum(a);
                ...
                % Obtain updated solution using the vecor a and the
                % previous solutions
                if size(stepReport.Residuals,2) == 1   %pressure
                updated(:,m_i) = state.pressure;
                else
                updated(:,m_i) = [state.pressure' state.c']';
                end
                
                updated_sum =  sum(a.*updated,2);
                
                
                if size(stepReport.Residuals,2) == 1   %pressure
                state.pressure = updated_sum;
                else
                state.pressure = updated_sum(1:size(updated_sum,2)/2);
                state.c = updated_sum(size(updated_sum,2)/2+1:size(updated_sum,2));
                end
        ...
        function [stateNext, updateReport, lineReport] = applyLinesearch(solver, model, state0, state, problem0, dx, drivingForces, varargin)
            assert(solver.linesearchReductionFactor < 1 & solver.linesearchReductionFactor > 0, ...
                    'NonLinearSolverAnderson.linesearchReductionFactor must be less than unity and positive.');
